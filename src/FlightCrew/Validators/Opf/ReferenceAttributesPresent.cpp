...
#include <stdafx.h>
#include "ReferenceAttributesPresent.h"
#include "Misc/Utilities.h"

namespace FlightCrew
{

std::vector<Result> ReferenceAttributesPresent::ValidateXml( const xc::DOMDocument &document )
{
    std::vector< QName > allowed_attributes;
    allowed_attributes.push_back( QName( "id", "" ) );
    allowed_attributes.push_back( QName( "type", "" ) );
    allowed_attributes.push_back( QName( "title", "" ) );
    allowed_attributes.push_back( QName( "href", "" ) );

    QName element_qname( "reference", OPF_XML_NAMESPACE );
    ...
    mandatory_attributes.push_back( QName( "type", "" ) );
